      ...
      function Neo4jD3(_selector, _options) {
        ...
        function appendTextToNode(node) {
          ...
          return node.append('text')
            ...
            .html(function (d) {
              var _icon = icon(d)
              for (var key in d.properties) { // ##显示的属性
                if (key === 'name') {
                  var item = d.properties[key]
                }
              }
              // console.log('item', item)
              return _icon ? '&#x' + _icon : item
            })
        }
        ...
        function resetNeoData(neo4jData, highlight) {
          nodes = []
          relationships = []

          resetWithNeo4jData(neo4jData, highlight)
        }
        ...
        function neo4jDataToD3Data(data) {
          ...
          if (data.graph) {
            ...
            data.graph.nodes.forEach(function (node) {
              // if (!contains(graph.nodes, node.id)) { // ## graph.nodes
              if (!contains(nodes, node.id)) { // 原图中不包括需要新增的节点
                if (!contains(graph.nodes, node.id)) { // 新增的图中不包括之前的循环已经新增的节点
            ...
            data.graph.relationships.forEach(function (relationship) {
              if (!contains(relationships, relationship.id)) {
                relationship.source = relationship.start.low //不是startNode！而是start.low！
                relationship.target = relationship.end.low //不是endNode！而是end.low！！
            ...
            console.log('__________________-------------->>>appendres', graph)
            ...
            data.relationships.forEach(function (relationship) {
              if (!contains(graph.relationships, relationship.id)) {
                relationship.source = relationship.start.low
                relationship.target = relationship.end.low
                graph.relationships.push(relationship)
              }
            })
        ...
        //  ##
        // function randomD3NeoData(cNode, oNode, iNode, maxNodesToGenerate) {
        //     var data = {
        //             nodes: [],
        //             relationships: []
        //         },
        ...
        //     for (i in oNode.responseJSON.results[0].data) {
        //         label = i.graph.nodes[0].labels;

        //         node = {
        //             id: i.graph.nodes[0].id,
        //             labels: [label],
        //             properties: i.graph.nodes[0].properties,
        //             x: oNode.x,
        //             y: oNode.y
        //         };
        ...
        //         relationship = {
        //             id: i.graph.relationships[0].id,
        //             type: i.graph.relationships[0].type,
        //             startNode: cNode.id,
        //             endNode: i.graph.nodes[0].id,
        //             properties: {
        //                 from: Date.now()
        //             },
        //             source: cNode.id,
        //             target: i.graph.nodes[0].id,
        //             linknum: s.relationships + 1 + i
        //         };
        ...
        function tickRelationshipsTexts() {
          relationshipText.attr('transform', function (d) {
            ...
            var rotatedPoint = rotatePoint(center, point, angle)
        ...
        function toString(d) {
          ...
          // s += ' (<id>: ' + d.id
          s += '\n' + '\t'
          s += 'name' + ': ' + '\t' + JSON.stringify(d.properties['name']) + '\n'
        ...
        function updateWithNeo4jData(neo4jData, highlight) {
        ...
        function updateInfo(d) {
          ...
          // info panel 设置
          Object.keys(d.properties).forEach(function (property) {
            if (property === 'name' || property === '名称') { // 去除掉摘要和受保护项
        ...
        function resetWithNeo4jData(neo4jData, highlight) {
